...
func TestUnaryExpression(t *testing.T) {
	...
	expr := checker.checkUnaryExpression(parser.UnaryExpression{
		Operator: testToken{kind: parser.QuestionMark},
		Operand:  parser.TokenExpression{Token: testToken{kind: parser.NumberKeyword}},
	})

	if len(checker.errors) != 0 {
		t.Fatalf("Expected no errors, got %#v", checker.errors)
	}
	if expr.Operator.Kind() != parser.QuestionMark {
...
func TestNoOptionValue(t *testing.T) {
	...
	checker.checkUnaryExpression(parser.UnaryExpression{
		Operator: testToken{kind: parser.QuestionMark},
		Operand:  parser.TokenExpression{Token: testToken{kind: parser.NumberLiteral, value: "42"}},
	})
