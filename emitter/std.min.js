...
export let createElement=s=>{let[a,t,i,c]=s.match(/^(\w[\w\-_]*)?(?:#(\w[\w\-_]*))?((?:\.\w[\w\-_]*)*)$/);if(!a)throw new Error("Invalid selector");let e=document.createElement(t||"div");if(i)e.id=i.slice(1);if(c)e.classList.add(...c.split(".").slice(1));return e}
export class DocumentBody extends Sum{}
export let getDocumentBody=d=>()=>(d instanceof NodePointer&&(d=d.get()),d.body&&new DocumentBody(d.body instanceof HTMLBodyElement?"Body":"Frame",d.body))
